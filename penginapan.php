...
<script src="assets/plugins/leaflet-routing/leaflet-routing-machine.min.js"></script>
    ...
    title.update = function () {
      this._div.innerHTML = '<h3> Eksplorasi Tempat Beristirahat | Distribusi Resort Penginapan  </h3> <h4> Kabupaten Buleleng</h4>'
    };
    title.addTo(map);
    ...
    L.Control.Legend = L.Control.extend({
      onAdd: function (map) {
        ...
        img.src = 'assets/img/legend/legendaresort.png';
    ...
    <?php
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                $info = $row["nama"];
                $lokasi = $row["lokasi"];
                $tipe = $row["tipe_penginapan"];
                $harga = $row["harga_per_malam"];
                $fasilitas = $row["fasilitas"];
                $rat = $row["rating"];
                echo "L.marker([$lat, $long]).addTo(map).bindPopup(' Nama : $info <br> Lokasi : $lokasi <br> Tipe Penginapan : $tipe <br> Harga per malam: $harga <br> Fasilitas: $fasilitas <br> Rating : $rat');";
            } 
        }
        else {
            echo "0 results";
